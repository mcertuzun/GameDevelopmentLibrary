...
public class FlyingTransaction : MonoBehaviour
{
    ...
    {
        IntVariableContainer tempCur = GameObject.FindGameObjectWithTag("Player").GetComponent<IntVariableContainer>();
        
        while (AddItemToPile && tempCur.GetCurrencyValue(relatedCurrency.Variable.Id) > 0 && brigde.RemainingCost > 0)
        {
            for (int i = 0; i < forCount && AddItemToPile && tempCur.GetCurrencyValue(relatedCurrency.Variable.Id) > 0 && brigde.RemainingCost > 0; i++)
            {
                if (!dontUseCurrencyForStorage)
                    relatedCurrency.Value++;

                tempCur.DecreaseCurrency(relatedCurrency.Variable.Id, 1);
